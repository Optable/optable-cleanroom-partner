...
BEGIN
  ...
  for r in c3 do
    ...
    ELSEIF (cn ILIKE 'id_utiq%') THEN
      INSERT INTO identifier(:snowflake_partner_source_schema_profiles) SELECT optable_partnership_v1.internal_schema.parse_utiq(identifier(:cn)), :match_id, :request_id FROM identifier(:source_table);
...
CREATE OR REPLACE FUNCTION optable_partnership_v1.internal_schema.parse_utiq(id VARCHAR)
RETURNS VARCHAR
AS
$$
  'utiq:' || id
...
$$
  CASE
    WHEN STARTSWITH(id, 'e') OR STARTSWITH(id, 'p') OR STARTSWITH(id, 'a') OR STARTSWITH(id, 'g') OR STARTSWITH(id, 'i4') OR STARTSWITH(id, 'i6') OR STARTSWITH(id, 's') OR STARTSWITH(id, 'r') OR STARTSWITH(id, 'f') OR STARTSWITH(id, 'n') OR STARTSWITH(id, 'z') OR STARTSWITH(id, 'id5') OR STARTSWITH(id, 'utiq') THEN
      CASE
